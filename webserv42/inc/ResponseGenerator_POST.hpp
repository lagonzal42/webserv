...
class ResponseGeneratorPOST
{
	public:
		...
		static std::string	postResponse(const Parser::Location& currentLoc, Request& req, std::vector<char *>& envp, const Parser::Server& currentServ, std::string& cleanPath, int cliVecPos);
		static std::string	postChunkedResponse(Request& req, int chunked);
		...
		static std::stack<int>	chunkedRequests;
};
